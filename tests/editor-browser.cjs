...
(async()=>{
 ...
 try{
  ...
  await page.setViewportSize({width:1024,height:700});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  ...
  console.log('Editor browser passed: legacy migration, storage zones, constant handles, drawing, movement, resize, undo/redo, locking, duplicate/delete, polygons, pan, import/export/reload, desktop and narrow desktop.');
