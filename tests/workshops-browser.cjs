...
(async()=>{
 ...
 try{
  ...
  await click('#wg-validate');let group=(await state()).workshops[0];assert.equal(group.validated,true);assert.equal(group.code,'AT-001');
  assert.equal(await page.locator('[data-workshop-surface]').count(),1);assert.equal(await page.locator('[data-workshop-item]').count(),0);assert.equal(await page.locator('[data-workshop-code]').textContent(),'AT-001');
  ...
  await click('#wg-validate');assert.equal((await state()).workshops[0].validated,false);assert.equal((await state()).workshops[0].code,'AT-001');await click('#wg-validate');
  ...
  const download=page.waitForEvent('download');await page.locator('#wg-settings summary').click();await click('#wg-export');const d=await download;const exported=JSON.parse(fs.readFileSync(await d.path(),'utf8'));assert.equal(exported.items.length,3);assert.equal(exported.workshops[0].code,'AT-001');assert.equal(exported.workshops[0].validated,true);
