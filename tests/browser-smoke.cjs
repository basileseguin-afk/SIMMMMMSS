...
(async()=>{
 ...
 try {
  ...
  await nav.vue(page,'plan');await page.locator('#zone-picker').selectOption('prepa');assert.match(await page.locator('#goulot-info').textContent(),/Montage/);
  ...
  await nav.vue(page,'vols');assert.equal(await page.locator('#flight-rows img').count(),0);assert.match(await page.locator('#flight-rows').textContent(),/<img src=x>/);
  ...
  await nav.vue(page,'plan');await nav.aller(page,'j-comparer');
  await click('#snap-a');assert.match(await page.locator('#compare').textContent(),/Commandes prêtes à l’heure/);
  const downloaded=page.waitForEvent('download');await nav.aller(page,'j-chiffres');await click('#btn-export');const download=await downloaded;
