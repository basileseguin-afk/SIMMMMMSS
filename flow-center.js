...
class FlowCenter{
 constructor(adapter){
  ...
  this.build();this.bind();this.refresh();this.status(warning||'Configuration locale prête. Classez les anciennes liaisons pour préciser leur rôle.');
 }
 get points(){return endpoints(this.a.zones());}
 status(message){document.getElementById('fc-status').textContent=message;}
 build(){
  this.host.innerHTML=`<div class="fc-heading"><div><span class="eyebrow">ORGANISER LES ÉCHANGES</span><h2>Centre des flux</h2><p>Une ligne par liaison orientée. Plusieurs départs et plusieurs arrivées sont possibles pour chaque emplacement.</p></div><div class="fc-actions"><button class="btn" id="fc-undo">Annuler</button><button class="btn" id="fc-redo">Rétablir</button><button class="btn" id="fc-export">Exporter</button><button class="btn" id="fc-import-button">Importer</button><input id="fc-import" type="file" accept=".json" hidden></div></div>
   <p class="fc-scope">Ces règles décrivent les échanges et les déplacements autorisés. Les temps de transport, les stocks et les effectifs ne sont pas encore recalculés à partir de ces liaisons.</p>
   ...
   <form id="fc-add" class="fc-card"><h3>Ajouter une liaison</h3><div class="fc-fields"><label>Flux<select id="fc-type">${this.typeOptions('material',false)}</select></label><label>Origine<select id="fc-from" required></select></label><label>Destination<select id="fc-to" required></select></label><label>Précision facultative<input id="fc-label" maxlength="200" placeholder="Ex. matériel propre"></label></div><button class="btn btn-play" type="submit">Ajouter la liaison</button><p class="mini-note">Pour plusieurs destinations, ajoutez une ligne pour chacune. Aucun article ni quantité à renseigner. Les stockages se créent dans la fiche du service sur le plan.</p></form>
   <div id="fc-list"></div>`;
