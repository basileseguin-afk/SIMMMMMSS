...
3. Grain et période :
   ...
   - temps FIXE par jour (runner, palette, cantine, désinfection des légumes…) ou au prorata d'autre chose que des vols : ne le convertis pas par défaut ; liste-le dans le compte rendu avec sa valeur en man-minutes par jour, sauf règle contraire dans la table. (Un temps fixe d'un service qui sert tout le monde, comme la légumerie, se saisit dans le simulateur, dans la case elle-même : « Travail de la journée ».)
...
5. Postes → services : applique la table ; plusieurs postes vers un même service × compagnie × classe s'additionnent. Un poste qui ne correspond à aucun service : demande-moi (l'ignorer ou le rattacher). Plonge et magasin n'ont pas de barème : rien sur ces services.
