...
class WorkshopGrid{
 ...
 build(){
  const panel=document.createElement('section');panel.id='workshop-panel';panel.hidden=true;panel.className='panel-content';panel.innerHTML=`<h2>Création des ateliers</h2><p id="wg-service">Choisissez un service sur la carte.</p><p class="mini-note">Cases de 50 × 50 cm théoriques. Le fond n’est pas calibré : cet aménagement est schématique, pas un relevé réel.</p><div id="wg-status" role="status" aria-live="polite"></div><div class="wg-actions"><button class="btn" id="wg-overview">Toute l’unité</button><button class="btn" id="wg-focus">Cadrer le service</button></div><div id="wg-controls" hidden><label>Atelier dans ce service<select id="wg-group"></select></label><div class="wg-actions"><button class="btn" id="wg-add-group">+ Atelier</button><button class="btn" id="wg-delete-group">Supprimer l’atelier</button></div><label>Nom de l’atelier<input id="wg-group-name" maxlength="120"></label><h3>Construire case par case</h3><div class="wg-tools">${[['select','Sélection / déplacement'],['table','Table'],['tapis','Chaîne'],['robot','Ligne robot'],['erase','Gomme']].map(([id,label])=>`<button class="btn" data-wg-tool="${id}" aria-pressed="false">${label}</button>`).join('')}</div><button class="btn" id="wg-new-item">+ Nouvel équipement</button><p class="mini-note">Cliquez-glissez pour peindre. Les cases voisines prolongent l’équipement sélectionné. La gomme retire les cases. En sélection, glissez pour déplacer.</p><div id="wg-items"></div><div id="wg-item-properties" hidden><label>Nom de l’équipement<input id="wg-item-name" maxlength="120"></label><div class="wg-actions"><button class="btn" id="wg-rotate">Pivoter 90°</button><button class="btn" id="wg-delete-item">Supprimer</button></div></div><button class="btn" id="wg-library">Bibliothèque de tables / chaînes…</button><p class="mini-note">Réutilisez aussi les modèles et assemblages de l’éditeur existant. Une copie indépendante est placée dans le service.</p></div><div class="wg-actions"><button class="btn" id="wg-undo">Annuler</button><button class="btn" id="wg-redo">Rétablir</button></div><details><summary>Grille et sauvegarde</summary><label>Taille visuelle d’une case (unités du dessin)<input type="number" id="wg-step" min="10" max="200" step="1"></label><p class="mini-note">Réglage commun à l’unité, indépendant d’une mesure réelle. Verrouillé dès qu’un équipement est placé.</p><div class="wg-actions"><button class="btn" id="wg-export">Exporter les ateliers</button><button class="btn" id="wg-import-button">Importer</button><input id="wg-import" type="file" accept=".json" hidden></div></details>`;
 ...
 render(){
  ...
  document.getElementById('wg-controls').hidden=!this.zone;document.getElementById('wg-service').textContent=this.zone?this.zone.nom+' · grille 50 cm schématique':'Choisissez un service sur la carte.';
